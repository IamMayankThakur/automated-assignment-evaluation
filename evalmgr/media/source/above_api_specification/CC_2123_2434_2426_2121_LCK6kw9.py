...
def add_user():
    ...
    # print("This is what i am receving ",user_details['username'])
    ...
    if(flag in [1,2]):
        ...
        return jsonify({"error":"form flag"}), 400
    print("AAj to gajab ka headache hua h bhai???????")
    if(check(str(user_details["password"]))):
        ...
        return jsonify({"error":"from if"}), req.status_code
    else:   
        return jsonify({"error":"from last"}), 400
    # Structure: {"user":{"username":"Aishwarya", "password":"12EaD145"}}
...
def new_ride():
    ...
    if((ride_info["source"] == ride_info["destination"])):
        return jsonify({}), 400
    
    if(int(ride_info["source"]) <= 1 or int(ride_info["source"]) >= 198):
        return jsonify({}), 400

    if(int(ride_info["destination"]) <= 1 or int(ride_info["destination"]) >= 198):
        return jsonify({}), 400

    #   Reading the DB
    payload = {"table":"User_Details","columns":["User_Name","Password"],\
        "where":"User_Name = '" + ride_info["created_by"]}
    req = requests.post("http://107.23.177.23/api/v1/db/read", json = {"query": payload})
