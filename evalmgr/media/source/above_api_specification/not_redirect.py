...
@app.route('/api/v1/rides/<source>/<destination>', methods = ['GET']) #CHECK FOR THE SOURCE AND DESTINATION
def upcoming_rides(source, destination):
    if (request.method == 'GET'):
        ...
        if (source >= 1 and source <= 198) and (destination >= 1 and destination <= 198):
            ...
            return "Source or Destination Entered is not correct!", 400
    else:
        return "GET method not used", 405
...
def delete_ride(rideId):
    if (request.method == 'DELETE'):
        ...
        return "DELETE method not used", 405
