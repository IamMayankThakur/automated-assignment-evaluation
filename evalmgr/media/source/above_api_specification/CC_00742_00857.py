...
class user(db.Model):
	...
	rides = db.relationship('ride',backref=db.backref('ride.created_by'),primaryjoin='user.username==ride.created_by',lazy='dynamic')
...
def table_write():
	in_insert = request.json['insert']
	in_column = request.json['column']
	in_table = request.json['table']
	query = db.update(in_table).values(in_column=in_insert)
...
def table_read():
	in_table=request.json['table']
	in_columns=request.json['columns']
	in_where=request.json['where']
	for column in columns:
		res = db.select([in_table.columns.in_column]).where(db.in_table.columns.in_where)
