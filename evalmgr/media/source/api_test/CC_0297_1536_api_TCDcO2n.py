...
def add_user():
	try:
		...
		return {},500 	
	re=requests.post("http://127.0.0.1:80/api/v1/db/read",json={"table":"User","columns":["username"],"where":"username="+"'"+un+"'"})
	if(re.json()):
		return {},405
	try:
		sha=int(pa,16)
	...
	if(len(pa)!=40):
		...
		requests.post("http://127.0.0.1:80/api/v1/db/write",json=j)
		return {},201

#remove user	
@app.route('/api/v1/users/<username>',methods=["DELETE"])
def remove_user(username):
	re=requests.post("http://127.0.0.1:80/api/v1/db/read",json={"table":"User","columns":["username"],"where":"username="+"'"+username+"'"})
	if(not re.json()):
		return {},405
	else:
...
def add_ride():
	...
	re=requests.post("http://127.0.0.1:80/api/v1/db/read",json={"table":"User","columns":["username"],"where":"username="+"'"+cr+"'"})
	if(not re.json()):
		return {},405
	else:
...
def remove_ride(rideId):
	re=requests.post("http://127.0.0.1:80/api/v1/db/read",json={"table":"Rides","columns":["rideId"],"where":"rideId="+"'"+rideId+"'"})
	if(not re.json()):
		return {},405
	else:
...
def ride_details(rideId):
	...
	if(not re.json()):
		...
		a=requests.post("http://127.0.0.1:80/api/v1/db/read",json={"table":"Rides","columns":["rideId","created_by","users","timestamp","source","destination"],"where":"rideId="+"'"+rideId+"'"})
		if(not a.json()):
			return {},204
...
def upcoming():
	...
	if(int(source)<1 or int(source)>ar or int(destination)<0 or int(destination)>ar):
		return {},405
	re=requests.post("http://127.0.0.1:80/api/v1/db/read",json={"table":"Rides","columns":["rideId","created_by","timestamp"],"where":"source="+"'"+str(source)+"' "+"and destination="+"'"+str(destination)+"'"})
...
def join_ride(rideId):
	...
	if(not re.json()):
		return {},405
	re1=requests.post("http://127.0.0.1:80/api/v1/db/read",json={"table":"Rides","columns":["rideId"],"where":"rideId="+"'"+rideId+"'"})
	if(not re1.json()):
		return {},405
	else:
		j={"flag":"update","columns":{"username":un,"rideId":rideId},"table":"Rides"}
		a=requests.post("http://127.0.0.1:80/api/v1/db/write",json=j)
		if(a.text=='405'):
			return {},405
	return {},200
