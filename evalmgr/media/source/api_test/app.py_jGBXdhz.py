...
def add_new_user():
    ...
    req_resp=requests.post("http://127.0.0.1:5000/api/v1/db/read",data = r, headers = headers)
    ...
    if(bool(temp)==True): #EMPTY DICTIONARY
        ...
        loaded_r = json.loads(r)
        headers = {'content-type': 'application/json'}
        req_resp=requests.post("http://127.0.0.1:5000/api/v1/db/write",data = r, headers = headers)
...
def create_new_ride():
    ...
    if(bool(temp)==False): #EMPTY DICTIONARY
        ...
        add_user={}
        add_user["insert"]=[str(user), str(src), str(dest), ride_id, str(time_stamp)]
...
def list_upcoming_rides():
    ...
    for i in temp:
        ...
        if temp_date_final>timestamp_final:
            ...
            notfinal["username"]=temp[i]["username"]
...
def join_existing_ride(rideid):
    ...
    if(bool(tempnew)==False): #EMPTY DICTIONARY
        ...
        loaded_r = json.loads(r)
        headers = {'content-type': 'application/json'}
        requests.post("http://127.0.0.1:5000/api/v1/db/write",data = r, headers = headers)
        return('', 200)
...
def write(): 
    ...
    if table=="rides": 
        ...
        comm="SELECT EXISTS (SELECT * FROM rides where src='"+ checksrc +"' and dest='"+ checkdest +"')"
        final=cur.execute(comm)  
        final=cur.fetchall()
        for i in final:
            for j in i:
                finalstr+=str(j)
        ...
        actualrideid=''
        actualtimestamp=''
        if finalstr=='0': #ride doesnt exist
            insert.append(insert[0])
        elif finalstr=='1':
            comm="select created_by from rides where src='"+ checksrc +"' and dest='"+ checkdest +"'"
            final=cur.execute(comm)  
            final=cur.fetchone()
            for i in final:
                actualcreatedby+=str(i)
            insert.append(actualcreatedby)
            comm="select ride_id from rides where src='"+ checksrc +"' and dest='"+ checkdest +"'"
            final=cur.execute(comm)  
            final=cur.fetchone()
            for i in final:
                actualrideid+=str(i)
            insert[3]=actualrideid
            comm="select timestamp from rides where src='"+ checksrc +"' and dest='"+ checkdest +"'"
            final=cur.execute(comm)  
            final=cur.fetchone()
            for i in final:
                actualtimestamp+=str(i)
            insert[4]=actualtimestamp
    stringcol=""
    stringinsert=""
    for i in column:
        stringcol+=", "+str(i)
