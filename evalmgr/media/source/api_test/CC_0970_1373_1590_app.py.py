...
class jtable(db.Model):
	...
	db.create_all()
	def __init__(self,ride_id,rider_list):
...
def write():
	...
	#join ride
		...
		username = data['username']
		create_ride= jtable(ride_id,username)	
		db.session.add(create_ride)
		db.session.commit()
		return make_response("",200)
	#delete ride
		...
		ride_id = data['ride_id']
		db.session.query(rides).filter(rides.ride_id == ride_id).delete()
		db.session.commit()
		return  make_response("deleted",200)
...
def read():
	...
	#Ride table
		...
		for ride in read_data:
			ride_data['ride_id'].append(ride.ride_id)
	...
	#rider_list table
		...
		for user in read_data:
			join_data['ride_id'].append(user.ride_id)
...
def list_rides():
	...
	for i in range(len(read_data["created_by"])):
		...
		if(source == read_data['source'][i] and destination == read_data['destination'][i] and time1>current_time):
			...
			upcoming.append({"ride_id":read_data['ride_id'][i],"username":read_data['created_by'][i],"timestamp":time})
	if(len(upcoming)!=0):
...
def ride_details(ride_id):
	...
	ride_id = int(ride_id)
	jlist=[]
	for i in range(len(join_data['ride_id'])):
		if ride_id==join_data['ride_id'][i]:
			jlist.append(join_data['rider_list'][i])
	for i in range(len(read_data["ride_id"])):
		if(ride_id == read_data["ride_id"][i]):
			time1 = datetime.strptime(read_data['timestamp'][i],"%a, %d %b %Y %H:%M:%S %Z" )
			time = datetime.strftime(time1,"%d-%m-%Y:%S-%M-%H")
			return jsonify({"ride_id":ride_id,"created_by":read_data['created_by'][i],"users":jlist,"timestamp":time,"source":read_data['source'][i],"destination":read_data['destination'][i]})
	else:
		return jsonify({}),204
...
def join_ride(ride_id):
	...
	if(username in user_data['username']):
		for i in range(len(ride_data['ride_id'])):
			time1 = datetime.strptime(ride_data['timestamp'][i],"%a, %d %b %Y %H:%M:%S %Z" )
			if(ride_id == ride_data['ride_id'][i] and time1>current_time and username != ride_data['created_by'][i]):
				write = requests.post('http://127.0.0.1:80/api/v1/db/write',json={"type":"jtable","ride_id":ride_id,"username":username})
				if(write):
					return jsonify({}),200
		else:
			return jsonify({}),204
...
def delete_ride(ride_id):
	...
	user_data = user_data.json()
	ride_id =int(ride_id)
	for i in range(len(user_data['ride_id'])):	
		if ride_id == user_data['ride_id'][i]:
			write = requests.post('http://127.0.0.1:80/api/v1/db/write',json={"type":"ride","ride_id":ride_id})
			if(write):
				return jsonify({}),200
			return jsonify({}),400
	else:
