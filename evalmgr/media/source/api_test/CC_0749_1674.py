	...
	try:	
		if method=="select":
			...
			if c.fetchone() is None:
				print(x[0],"column with value",x[1],"doesnt exist")
		...
		if method=="selectdel":
			...
			for i in range(len(r)):
				...
				if str(r[i][1])!=str(z[1][1]):
					'''print("User has already joined the ride")
					break
			...
			res=c.fetchall()
			if len(res)==0:
				print(x[0],y[0],"columns with values",x[1],y[1],"don't exist")
				return Response(status=404)
				...
				if res2.status_code==404:
					...
					if res4.status_code==200:
						...
						if res3.status_code==200:
							print(tryjoin ,"joined ride succesfully")
					...
					print("User who created ride cant join ride")
	...
	try:
		...
		if res1.status_code==404:
			...
			if valtime!=0:
				...
				if res3.status_code==200:
					print(username ,"created ride succesfully")
					return json.dumps({}),201
				else:
					return json.dumps({}),400
					
			else:
				print("incorrect timeformat!")
				return json.dumps({}),400
	except:
		return Response(status=400)
...
def list_updetails():
	...
	try:
		...
		if(Place.has_value(int(src))==True and Place.has_value(int(dest))==True):
			...
			res1=requests.post('http://52.86.225.139:80/api/v1/db/read',json=task1)
			if res1.status_code==404:
				print("No upcoming ride for given source and destination")
				return json.dumps({}),204
