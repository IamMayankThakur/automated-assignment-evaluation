...
def rideExistsBetween(source, destination):
...
def userInRide(username, ride_id):
	r = requests.post(db_read_url, json={"table":"RideDetails", "columns":["riders_list"], 
												"where":"ride_id="+ride_id})
	print("YO ALL:", r.json())
	l = r.json()[0][0]
	l = l.split(",")[:-1]
...
def keygen():
	...
	key = "".join([choice(l) for _ in range(40)])
	return key
...
def add_user():
	...
	try:
		...
		abort(400, "Invalid JSON request object")
...
def remove_user(username):
	if userExists(username):
		if not userInAnyRide(username):
			removeUser(username)
			return jsonify({}), 200
		else:
			abort(400, "User is part of a ride.")
	else:
...
def create_new_ride():
	...
	try:
		...
		if not validAreas([source, destination]):
			abort(400, "Invalid source or destination")
		elif not userExists(created_by):
			abort(400, "user does not exist")
		elif wrongTime(timestamp):
			abort(400, "Wrong timestamp provided")
		else:
...
def upcoming_rides():
	try:
		...
		abort(400, "Invalid JSON request object")
	if rideExistsBetween(source, destination):
...
def ride_details(rideId):
	if request.method == "GET":			# Get ride details
		...
		elif userInRide(join_user, rideId):
			abort(400, "User already in ride")
		else:
