    ...
    if (request.method=='PUT'):
        ...
        if not user_in_db:
            ...
            if check:
                ...
                user = requests.post('http://localhost:5000/api/v1/db/write',json=query)
                return jsonify({}),201
    ...
    if (request.method == 'POST'):
        ...
        if user_in_db==None:
            ...
            if source!=destination:
                ...
                ride = requests.post('http://localhost:5000/api/v1/db/write',json=query)
                return jsonify({}),201
            else:
                return jsonify({}),400
    else:
...
def del_ride(ride_id):
    if(request.method=='DELETE'):
        ...
        query = {"op":4}
        del_user = requests.post('http://localhost:5000/api/v1/db/write',json={"query":query})
        return jsonify({}),200
    else:
            return jsonify({}),400
