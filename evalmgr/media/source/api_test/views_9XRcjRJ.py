...
http://0.0.0.0:80/api/v1/users/<nishi>


API 3
add ride post

http://0.0.0.0:80/api/v1/rides
...
API  4 GET
 http://0.0.0.0:80/api/v1/rides?source=10&destination=11

API 5 GET  given rideID


http://0.0.0.0:80/api/v1/rides/1


API 6 JOIN RIDE POST
http://0.0.0.0:80/api/v1/rides/<ride_query_ID>
...
def add_user():
	...
	r = requests.put('http://0.0.0.0:80/api/v1/db/write', json=data)
	if(r.text=="0"):
...
def delete_user(username):
	...
	data["flag"]="2"
	data["username"]=username
	r = requests.delete('http://0.0.0.0:80/api/v1/db/write', json=data)
	if(r.text=="0"):
...
def create_new_ride():
	...
	ride["flag"]="3"
	r = requests.post('http://0.0.0.0:80/api/v1/db/write', json=ride)
	#return r.text
	if(r.text=="0"):
...
def rides():
	...
	query["time"]=act_time	
	r = requests.post('http://0.0.0.0:80/api/v1/db/read',json=query)
	#return "hjj"	
	if(r.text=="0"):
...
def join_ride(ride_query_ID):
	...
	r = requests.post('http://0.0.0.0:80/api/v1/db/write', json=data)
	if(r.text=="0"):
