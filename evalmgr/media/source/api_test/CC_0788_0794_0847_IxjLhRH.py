...
def create_user():
    ...
    if(bool(pattern.match(data['password']))!=True):
        abort(405)
    print(data["name"])
    requests.post("http://18.208.227.152:80/api/v1/db/write", json={"api":"1","name":data['name'],"password":data['password']},headers=headers)
    return {},201
...
def remove_user(username):
    if db.session.query(User).filter_by(name=username).count():
        requests.post("http://18.208.227.152:80/api/v1/db/write", json ={"api":"2","name":username})
        return {},200
    else:
        abort(405)
...
def create_rider():
    ...
    if request.method == 'POST':
        if ((db.session.query(User).filter_by(name=data['created_by']).count()) and (data['source'] in d )and data['destination'] in d):
            requests.post("http://18.208.227.152:80/api/v1/db/write", json ={"api":"3","created_by":data['created_by'], "timestamp":data['timestamp'], "source":data['source'], "destination":data['destination']},headers=headers)
            return {},201
    else:
        abort(405)
...
def list_ride(ride_id):
    if request.method == 'GET':
        if db.session.query(Rider).filter_by(id=ride_id).count():
            r=requests.post("http://18.208.227.152:80/api/v1/db/read", json ={"api":"5","ride_id":ride_id},headers=headers)
            return r.json(),200
        else:
            return {},204
        ...
        if db.session.query(Rider).filter_by(id=ride_id).count() and db.session.query(User).filter_by(name=request.get_json()['username']).one():
            requests.post("http://18.208.227.152:80/api/v1/db/write", json ={"api":"6","ride_id":ride_id,"shared_by":request.get_json()['username']},headers=headers)
            return {},201
        else:
            return {},204
...
if __name__ == '__main__':
    app.run(host='18.208.227.152',port=80,debug=True)
