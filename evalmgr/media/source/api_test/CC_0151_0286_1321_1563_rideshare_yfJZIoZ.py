...
def add_user():
    ...
    if(count == 40):
        ...
        if(response == []):
            ...
            response_msg = {
                "response": "Can not create new user. Username already exists"}
            status = 400
    else:
        response_msg = {
            "response": "Can not create new user. Wrong password format"}
        status = 400
...
def ride_create_list():
    if(request.method == "POST"):
        ...
        if (in_flag or name_response == [] or source_response == [] or destination_response == [] or dt < now):
