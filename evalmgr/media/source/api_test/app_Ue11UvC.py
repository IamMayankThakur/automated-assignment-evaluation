...
# Team name: CC_0158_0115_0108
ip = "52.207.1.214"
# Implemented Nginx as a reverse proxy on port 80 - Attached config file

# API 1
# Add user
@app.route("/api/v1/users",methods=['PUT'])
def add_user():
    print("------- [API 1] ------- [{}] ------- START ADD USER -------".format(str(datetime.now())))
    print("------- [API 1] ------- [{}] ------- REQUEST: {} -------".format(str(datetime.now()), request.get_json()))
    if(request.method!='PUT'):
        print("------- [API 1] ------- [{}] ------- [RESPONSE:405] -------".format(str(datetime.now())))
        abort(405)
    req_params = request.get_json()
    if req_params == None:
        print("------- [API 1] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    ...
    try:
        ...
        if x.status_code != 200:
            print("------- [API 1] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
            abort(400)
        print("------- [API 1] ------- [{}] ------- [RESPONSE:201] -------".format(str(datetime.now())))
        return("",201)
...
def delete_user(username):
    print("------- [API 2] ------- [{}] ------- START DELETE USER -------".format(str(datetime.now())))
    print("------- [API 2] ------- [{}] ------- REQUEST: {} -------".format(str(datetime.now()), username))
    if(request.method!='DELETE'):
        print("------- [API 2] ------- [{}] ------- [RESPONSE:405] -------".format(str(datetime.now())))
        abort(405)
...
def create_ride():
    print("------- [API 3] ------- [{}] ------- START CREATE RIDE -------".format(str(datetime.now())))
    print("------- [API 3] ------- [{}] ------- REQUEST: {} -------".format(str(datetime.now()), request.get_json().values()))
    if(request.method!='POST'):
        print("------- [API 3] ------- [{}] ------- [RESPONSE:405] -------".format(str(datetime.now())))
        abort(405)
    req_params=request.get_json()
    if req_params == None:
        print("------- [API 3] ------- [{}] ------- [RESPONSE:405] -------".format(str(datetime.now())))
    ...
    find_resp = requests.post("http://" + ip + "/api/v1/db/read", json=find_query)
    if len(find_resp.json()) == 0:
        print("------- [API 3] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    ...
    if users_response.status_code != 200:
        print("------- [API 3] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    print("------- [API 3] ------- [{}] ------- [RESPONSE:201] -------".format(str(datetime.now())))
    return ("",201)
...
def getRides():
    print("------- [API 4] ------- [{}] ------- START GET UPCOMING RIDES -------".format(str(datetime.now())))
    print("------- [API 4] ------- [{}] ------- REQUEST: {} -------".format(str(datetime.now()), (request.args.get('source'), request.args.get('destination'))))
    ...
    if len(read_result.json()) == 0:
        print("------- [API 4] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    else:
        net_response = []
        now = datetime.now()
        ...
        if len(net_response) == 0:
            print("------- [API 4] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
            abort(400)
        print("------- [API 4] ------- [{}] ------- [RESPONSE:200] -------".format(str(datetime.now())))
...
def getRideDetails(rideId):
    print("------- [API 5] ------- [{}] ------- START GET RIDE DETAILS -------".format(str(datetime.now())))
    print("------- [API 5] ------- [{}] ------- REQUEST: {} -------".format(str(datetime.now()), rideId))
    ...
    if read_result.status_code != 200:
        print("------- [API 5] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    if len(read_result.json()) == 0:
        print("------- [API 5] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
        ...
        print("------- [API 5] ------- [{}] ------- [RESPONSE:200] -------".format(str(datetime.now())))
...
def addUser(rideid):
    print("------- [API 6] ------- [{}] ------- START JOIN EXISTING RIDE -------".format(str(datetime.now())))
    print("------- [API 6] ------- [{}] ------- REQUEST: {} -------".format(str(datetime.now()), (request.get_json(),rideId)))
    req_params = request.get_json()

    if req_params == None:
        print("------- [API 6] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    username = req_params["username"]
    ...
    if add_user_response.status_code != 200:
        print("------- [API 6] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    print("------- [API 6] ------- [{}] ------- [RESPONSE:204] -------".format(str(datetime.now())))
    return ('', 204)
...
def delete_ride(rideid):
    print("------- [API 7] ------- [{}] ------- START DELETE RIDE -------".format(str(datetime.now())))
    print("------- [API 7] ------- [{}] ------- REQUEST: {} -------".format(str(datetime.now()), rideid))
    find_query = {"table": "rides", "columns": ["rideid"], "where": ["rideid={}".format(rideid)]}
    find_resp = requests.post("http://" + ip + "/api/v1/db/read", json=find_query)
    if len(find_resp.json()) == 0:
        print("------- [API 7] ------- [{}] ------- [RESPONSE:400] -------".format(str(datetime.now())))
        abort(400)
    delete_query = {"where": ["rideid={}".format(rideid)], "table": "rides"}
    delete_resp = requests.delete("http://" + ip + "/api/v1/db/write", json=delete_query)
    print("------- [API 7] ------- [{}] ------- [RESPONSE:200] -------".format(str(datetime.now())))
    return (jsonify(), 200)
