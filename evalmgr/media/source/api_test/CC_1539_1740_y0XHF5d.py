...
def write_db():
    ...
    try:
        if(data['table']=='User' and data['action']=='New_user'):
            ...
            # session.delete(ride)
            session.query(Ride2).filter(Ride2.rideIid==rideId).delete(synchronize_session=False)
            session.commit()
            respose_ = Response(status = 200)
            return respose_
...
def add_user():
    ...
    if(len(data)==2):
        ...
        if(pas>0):
            ...
            check_user = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/read",json = myobj)            
            if(str(check_user.status_code)=="400"):
                response = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/write",json = myobj)
...
def remove_user(username):
    ...
    if(str(check_user.status_code)=="200"):
        ...
        ride_status = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/read",json = ride_present)
...
def new_ride():
    ...
    max_id = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/read",json = ride_obj)
...
def list_ride():
    ...
    if(place_present==1):
        ...
        # return myobj
        list_ride = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/read",json = myobj)
        ...
        for my_lis in temp:
            ...
            y=datetime.datetime.now()
            if(x>y):
                ret_dict={"rideId":rideId,"username":user_name,"timestamp":date}
                ret.append(ret_dict)
...
def join_ride(rideId):
    ...
    check_ride = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/read",json = rideobj)
    if(str(check_ride.status_code)=="200"):
        ride_status_obj = {"table":"Join","action":"ride_status","rideId":rideId,"user_name":user_name}
        ride_status = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/read",json = ride_status_obj)
        if(str(ride_status.status_code)=="200"):
            response_ = Response(status=400)#user already added to the ride
            return response_
        else:
            join_ride = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/write",json = ride_status_obj)
...
def ride_detail(rideId):
    ...
    if(str(check_ride.status_code)=="200"):
        ...
        ride_detail1 = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/read",json = rideobj1)
...
def ride_delete(rideId):  
    ...
    # return str(check_ride.status_code)
    if(str(check_ride.status_code)=="200"):
        del_ride = requests.post("http://ec2-52-3-114-196.compute-1.amazonaws.com/api/v1/db/write",json = rideobj)
        # ride =session.query(Ride2).get(ride_id)
