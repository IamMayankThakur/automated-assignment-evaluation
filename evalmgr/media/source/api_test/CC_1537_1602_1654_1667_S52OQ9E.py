...
def add():
	...
	res=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/write",json={"insert":d,"column":["name","pass"],"table":"users","indicate":"0"})	
...
def insert_rider():
	...
	read_res=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/read",json={"insert":d,"column":["name","pass"],"table":"users","where":["name"]})
	if(read_res.json().get("response")==0):
		abort(400,"user name doesn't exists")

	res=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/write",json={"insert":d,"column":["name","timest","source","desti"],"table":"rides","indicate":"0"})	
	if(res.json()==0):
		abort(400,"user already exists")
...
def remove(name):
	if(request.method!="DELETE"):
		abort(405,"method not allowed")
	res=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/write",json={"table":"users","delete":name,"column":"name","indicate":"1"})
...
def delete_rideId(rideId):
	if(request.method!="DELETE"):
		abort(405,"method not allowed")
	res=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/write",json={"table":"rides","delete":rideId,"column":"rideid","indicate":"1"})
	if(res.json()==0):
...
def join_ride(rideId):
	...
	d=[rideId,name]
	rideid_check=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/read",json={"insert":d,"column":["rideid","name","source","desti"],"table":"rides","where":["rideid"]})
	if(rideid_check.json().get("response")==0):
		abort(400,"ride id doesn't exists")
	
	res=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/write",json={"insert":d,"column":["id","name"],"table":"rideusers","indicate":"0"})
	if(res.json()==0):
		abort(400,"rideId does not  exists")
	elif(res.json()==1):
...
def ride_details(rideId):
	...
	d=[rideId]
	user_list=[]
	rideid_check=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/read",json={"insert":d,"column":["rideid","name","source","desti","timest"],"table":"rides","where":["rideid"]})
	if(rideid_check.json().get("response")==0):
		abort(400,"rideId does not  exists")
	elif(rideid_check.json().get("response")==1):
		
		joined_users_check=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/read",json={"insert":d,"column":["id","name"],"table":"rideusers","where":["id"]})
...
def upcoming_rides():
	...
	src_dest_check=requests.post("ec2-34-201-201-196.compute-1.amazonaws.com/api/v1/db/read",json={"insert":d,"column":["rideid","name","source","desti","timest"],"table":"rides","where":['source','desti']})			
