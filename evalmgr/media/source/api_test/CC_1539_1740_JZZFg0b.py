...
def write_db():
    ...
    try:
        if(data['table']=='User' and data['action']=='New_user'):
            ...
            new_ride = Ride2(rideId=data['user_data'][-1],user_name=data['user_data'][0],source=data['user_data'][2],destination=data['user_data'][3],stamp=Datetime)
            ...
            # session.delete(ride)
            session.query(Ride2).filter(Ride2.rideId==rideId).delete(synchronize_session=False)
            session.commit()
            respose_ = Response(status = 200)
            return respose_
            ...
            joinride =Join(rideId=rideId,user_name=username)
...
def read_db():
    ...
    if(data['action']=='New_user'  or data['action']=='delete_user'):
        ...
        # conds = [Ride2.source==source,Ride2.destination==destination,Ride2.stamp>now]
        query = db.select([Ride2]).where(db.and_(Ride2.source==source,Ride2.destination==destination,Ride2.stamp>now))
        results = connection.execute(query).fetchall()
        ret=str(results)
        response_ = Response(response=ret,status=200)
        return response_
    elif(data['action']=='get_max_ride_id'):
        max_id =session.query(func.max(Ride2.rideId)).scalar()
        if(max_id ==None):
        ...
        ride_status = session.query(Ride2).get(rideId)
        ...
        rideId = data['rideId']
        user_name = data['user_name']
        status = session.query(Join).get({'rideId':rideId,'user_name':user_name})        
        if(status==None ):
            response = Response(status = 400) # returns 400 when a ride is not present 
            return response
        else:
        ...
        rideId = data['rideId']
        connection = engine.connect()
        query = db.select([Ride2]).where(Ride2.rideId == rideId)
        results = connection.execute(query).fetchall()
        query1 = db.select([Join]).where(Join.rideId == rideId)
...
def add_user():
    ...
    if(len(data)==2):
        ...
        if(pas>0):
            ...
            check_user = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = myobj)            
            if(str(check_user.status_code)=="400"):
                response = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/write",json = myobj)
...
def remove_user(username):
    ...
    if(str(check_user.status_code)=="200"):
        ...
        ride_status = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = ride_present)
        ...
        if(len(my_str1)==2):
            ...
            for my_lis in temp:
                ...
                if(x>y):
                    # ret_dict={"ride_id":ride_id,"username":user_name,"timestamp":date}
                    ret.append(x)
            if(len(ret)!=0):
...
def new_ride():
    ...
    max_id = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = ride_obj)
    rideId=int(max_id.text)+1
    myobj ={"table":"Ride","action":"New_ride","user_data":[user_name,timestamp,source,destination,rideId]}     
    check_user = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = myobj)
    if(str(check_user.status_code)=="200"):
        response = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/write",json = myobj)
...
def list_ride():
    ...
    if(place_present==1):
        ...
        # return myobj
        list_ride = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = myobj)
        ...
        for my_lis in temp:
            ...
            y=datetime.datetime.now()
            if(x>y):
                ret_dict={"rideId":int(rideId),"username":user_name,"timestamp":date}
                ret.append(ret_dict)
...
def join_ride(rideId):
    data = request.get_json()
    user_name = data['username']
    rideobj = {"action":"check_ride","rideId":rideId,"user_name":user_name}
    check_ride = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = rideobj)
    if(str(check_ride.status_code)=="200"):
        ride_status_obj = {"table":"Join","action":"ride_status","rideId":rideId,"user_name":user_name}
        ride_status = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = ride_status_obj)
        if(str(ride_status.status_code)=="200"):
            response_ = Response(status=400)#user already added to the ride
            return response_
        else:
            join_ride = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/write",json = ride_status_obj)
...
def ride_detail(rideId):
    rideobj = {"action":"check_ride_detail","rideId":rideId}
    # return rideobj
    check_ride = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = rideobj)
    #return str(check_ride.status_code)
    if(str(check_ride.status_code)=="200"):
        rideobj1 ={"action":"check_ride_detail1","rideId":rideId}
        ride_detail1 = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = rideobj1)
        # return str(ride_detail1.text)
        my_str1 = ride_detail1.text
        # return str(my_str1)
        # return str(len(my_str1))
        ret1 = my_str1.split('&')
        users = ret1[1].split('),')
        users1=[]
        new1 = []
        ...
        ret = json.dumps(details_ride)
        return ret
    else:
        response_ = Response(status=400)
        return response_ #invalid ride_id
...
def ride_delete(rideId):  
    rideobj = {"table":"Ride","action":"delete_ride","rideId":rideId}

    check_ride = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/read",json = rideobj)
    # return str(check_ride.status_code)
    if(str(check_ride.status_code)=="200"):
        del_ride = requests.post("http://ec2-52-204-109-189.compute-1.amazonaws.com/api/v1/db/write",json = rideobj)
        # ride =session.query(Ride2).get(ride_id)
