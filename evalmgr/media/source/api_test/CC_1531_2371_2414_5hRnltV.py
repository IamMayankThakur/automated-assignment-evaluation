...
class Ride(db.Model):
    ...
    src_adr = db.Column(db.Integer,nullable=False)
    dest_adr = db.Column(db.Integer,nullable=False)
    created = db.Column(db.String, nullable=False)
    
    def __repr__(self):
        return '\n{"rideId": %d ,"username": %s, "timestamp" : %s}' % (self.ride_id,self.created,self.time)
"""
...
def add_user():
    if (request.method=="PUT"):
        ...
        res=requests.post(url="http://127.0.0.1:5000/api/v1/db/read", json=mydict)
        ...
        if res.status_code == 204:
            if(re.match(r'[0-9a-fA-F]{40}$',_password) is None):
                ...
                data=requests.post(url="http://127.0.0.1:5000/api/v1/db/write", json=sqlquery)
                return jsonify({}),201
    else:
...
def user_delete(username):
    ...
    if(request.method=="DELETE"):
        ...
        if (res.status_code==200):
            ...
            result=requests.post(url="http://127.0.0.1:5000/api/v1/db/write", json=sqlquery)
    ...
    if request.method == "DELETE":
        #try:
            ...
            rflag = Ride.query.filter_by(created=username).first()
            
            uflag = User.query.filter_by(username=username).first()

            if rflag is not None or uflag is None:
                return jsonify({}),400
            #print(uflag["user_id"])
            db.session.delete(uflag)
            #UserRide.query.filter_by(username=username).delete()
            db.session.commit()
    ...
    if(request.method=="GET"):
        ...
        if existing_user:
            ...
            for record in records:
                recordObject = { "rideId":record.ride_id,
...
def join_ride(rideId):
    if(request.method=="POST"):
        ...
        num=requests.post(url="http://127.0.0.1:5000/api/v1/db/read", json=myid)
        if (num.status_code == 200):
            ...
            if(res.status_code==200):
                ...
                if(createdby[0] != usern):
                    ...
                    req=requests.post(url="http://127.0.0.1:5000/api/v1/db/write", json=sql) #adding users to join ride
...
def delete_ride(id):
    if(request.method=="DELETE"):
        ...
        data = requests.post(url="http://127.0.0.1:5000/api/v1/db/read", json=mydict) #checks if ride exist
        if (data.status_code==200):
            sqlquery={"insert":id, "table":"ride","query":"delete"}
            res=requests.post(url="http://127.0.0.1:5000/api/v1/db/write", json=sqlquery) 
            if res.status_code==200:
