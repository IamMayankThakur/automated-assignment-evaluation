...
def add_ride():
	...
	if(source not in area_list or destination not in area_list or source==destination):
		abort(400,description="Bad Request")
...
def list_rides():
	...
	for i in cat:
		...
		if(date_type1>present_time):
			x={"rideId":i[0],"created_by":i[1],"timestamp":i[2]}
			rides.append(x)
...
def full_details(rideId):
	...
	x={"rideId":ride_details[0], "created_by" :ride_details[1],"users" : l1,"timestamp":ride_details[3],"source":ride_details[4],"destination":ride_details[5]} 
