...
def delete_user(username):
	...
	try:
		response = requests.post('http://0.0.0.0:5000/api/v1/db/read',json={'table':'tbl_user','columns':'passwords','where':['username',username],'gofor':'username'})
		json_response = response.json()
		if(int(json_response)==0):
			#return jsonify('Username already exists'),400
			return jsonify({}),400
...
@app.route('/api/v1/rides',methods=['PUT','POST','DELETE','HEAD','OPTIONS'])
def add_ride():
	try:
...
def foo():
	try:
		...
		if summary==None or change==None:
			...
			pass
		response = requests.post('http://0.0.0.0:5000/api/v1/db/read',json={'table':'places','columns':'AreaName','where':['AreaNo',summary],'gofor':'username'})
		json_response = response.json()
		if(int(json_response)==0):
			#return jsonify('Username already exists'),400
			return jsonify({}),400

		response1 = requests.post('http://0.0.0.0:5000/api/v1/db/read',json={'table':'places','columns':'AreaName','where':['AreaNo',change],'gofor':'username'})
		...
		if (str(resp.text)).strip() == '[]':
			return resp.text, 204
