...
def join_ride(rideId):
	...
	try:
		...
		res1=requests.post('http://127.0.0.1:5000/api/v1/db/read',json=task1)
		...
		if res1.status_code==404:
			...
			if res1.status_code==404:
				...
				res2=requests.post('http://127.0.0.1:5000/api/v1/db/read',json=task1)

				if res2.status_code==404:

					task3={"method":"joinrides","table":"ride","columns":["ujname","rideid"],"where":"rideid="+rideId+",ujname='"+tryjoin+"'"}

					res4=requests.post('http://127.0.0.1:5000/api/v1/db/read',json=task3)
					
					if res4.status_code==200:
						task2={"method":"insert" ,"insert":(tryjoin,rideId),"column":("ujname","rideid"),"table":"ride"}
						res3=requests.post('http://127.0.0.1:5000/api/v1/db/write',json=task2)
						if res3.status_code==200:
							(tryjoin ,"joined ride succesfully")
...
def delride(rideId):
	...
	try:
		...
		if res1.status_code==404:
			...
			res2=requests.post('http://127.0.0.1:5000/api/v1/db/write',json=task2)
			task3={"method":"delride","table":"ride","insert":(rideid),"column":["rideid"]}
			res3=requests.post('http://127.0.0.1:5000/api/v1/db/write',json=task3)
			if res3.status_code==200:
...
def list_ride(rideId):
	...
	try:
		...
		if res1.status_code==404:
			...
			res2=requests.post('http://127.0.0.1:5000/api/v1/db/read',json=task2)

			task3={"method":"selectuj","table":"ride","columns":["ujname"],"where":"rideid='"+rideId+"'"}
			res3=requests.post('http://127.0.0.1:5000/api/v1/db/read',json=task3)
	...
	try:
		...
		if(validationP(password)==True):
			...
			if res1.status_code==404:
				...
				res=requests.post('http://127.0.0.1:5000/api/v1/db/write',json=task2)
				(res)
...
def del_user(username):
	...
	try:
		...
		if res1.status_code==404:
			...
			res4=requests.post('http://127.0.0.1:5000/api/v1/db/write',json=task3)

			task4={"method":"delete","insert":(username),"column":("ujname"),"table":"ride"}
			res5=requests.post('http://127.0.0.1:5000/api/v1/db/write',json=task4)

			if res5.status_code==200:
				(username , "Deleted succesfully")
				return json.dumps({}),200
			else:
				return json.dumps({}),400
	...
	try:
		...
		if res1.status_code==404:
			...
			if valtime!=0:
				...
				task2={"method":"insert" ,"insert":(username,ts,int(src),int(dest)),"column":("created_by","timestamp","source","destination"),"table":"cride"}
				
				res3=requests.post('http://127.0.0.1:5000/api/v1/db/write',json=task2)
				
				if res3.status_code==200:
					(username ,"created ride succesfully")
					return json.dumps({}),201
				else:
					return json.dumps({}),400
...
def list_updetails():
	...
	try:
		...
		r=requests.post('http://127.0.0.1:5000/api/v1/db/read',json=t)
		...
		if(Place.has_value(int(src))==True and Place.has_value(int(dest))==True):
			...
			#res=requests.post('http://127.0.0.1:5000/api/v1/read',json=task1)
