...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///cc4.sqlite'
db = SQLAlchemy(app)
...
class ride_details(db.Model):
                ...
                ride_Id = db.Column('ride_Id',db.Integer, primary_key=True,autoincrement=True) 
...
def insert():
        ...
        if(request.json['type']=="del_user"):
                ...
                print('insert',insert)
                ride_details.query.filter_by(ride_Id=int(insert[0])).delete()
                ...
                str4 = str2.strip(',')
                if(column[0]=="users"):
                        execstr = 'UPDATE '+table+' SET users='+'"'+ insert[0] +'"'+' where ride_Id='+'"'+ insert[1] +'"'
                        print(execstr)
                        engine.execute(execstr)
                        db.session.commit()
                        return {},200
...
def read():
        ...
        elif(new_where[0]=="ride_Id"):
                if(ride_details.query.filter_by(ride_Id=new_where[1]).first()):
                        execstr = 'SELECT '+str4+ ' FROM ' +table + ' WHERE '+new_where[0]+' = ' +'"'+ new_where[1] +'"'
                        print(execstr)
...
def adduser():
        ...
        if(len(pin)==40 and re.findall("^[0-9a-fA-F]+$",pin)):
                response=requests.post('http://0.0.0.0:80/api/v1/db/write',json={"table":"users_det","column":["username","password"],"insert":[user,pin],"type":''})
...
def removeuser(name):
        print(name)
        type="del_user"
        response=requests.post('http://0.0.0.0:80/api/v1/db/read',json={"table":"users_det","columns":["username","password"],"where":'username='+name})
        if(response.status_code == 200):
                resp=requests.post('http://0.0.0.0:80/api/v1/db/write',json={"table":"users_det","column":["username"],"insert":[name],"type":type})

                
                if(resp.status_code==400):
                        return jsonify("this user cannot be deleted as he is associated with a ride"),400
                else:
                        return {},200
                
                
        elif(response.status_code==400):
                resp=requests.post('http://0.0.0.0:80/api/v1/db/write',json={"table":"ride_details","column":["users"],"insert":[name],"type":type})
...
def new_ride():
        ...
        if(time > pres) :
            print("yes")
            response_check=requests.post("http://0.0.0.0:80/api/v1/db/read",json={"table":"users_det","columns":["username","password"],"where":'username='+created_by})
            ...
            if(response_check.status_code==200):
                    ...
                    if(int(source)>=1 and int(source)<=len(leng) and int(destination)>=1 and int(destination)<=len(leng) and int(source)!=int(destination)):
                        ...
                        if(len(db.session.query(ride_details).all())==0):
                                response=requests.post("http://0.0.0.0:80/api/v1/db/write",json={"table":"ride_details","column":["created_by","timestamp","source","destination","ride_Id","users"],"insert":[created_by,timestamp,source,destination,len(db.session.query(ride_details).all())+1,created_by],"type":''})
                                return {},response.status_code
                        else:
                                response=requests.post("http://0.0.0.0:80/api/v1/db/write",json={"table":"ride_details","column":["created_by","timestamp","source","destination","ride_Id","users"],"insert":[created_by,timestamp,source,destination,(db.session.query(db.func.max(ride_details.ride_Id)).scalar())+1,created_by],"type":''})
                                return {},response.status_code 
                    else:
                        return {},400
            else:
...
def deleteride(name):
                id=int(name)
                type="del_ride"
                response=requests.post('http://0.0.0.0:80/api/v1/db/read',json={"table":"ride_details","columns":["created_by","timestamp","source","destination","ride_Id"],"where":'ride_Id='+str(id)})
                ...
                for rowproxy in response.json():
                        ...
                        a.append(d)
                if(len(a)>0):
                        res=requests.post('http://0.0.0.0:80/api/v1/db/write',json={"table":"ride_details","column":["ride_Id"],"insert":[str(id)],"type":type})
...
def list_upcoming():
                ...
                response_check=requests.post("http://0.0.0.0:80/api/v1/db/read",json={"table":"ride_details","columns":["created_by","timestamp","source","destination","ride_Id"],"where":'source='+source+' and destination='+destination})
...
def join_ride(id):
        ...
        response=requests.post('http://0.0.0.0:80/api/v1/db/read',json={"table":"ride_details","columns":["created_by","timestamp","source","destination","ride_Id","users"],"where":'ride_Id='+str(id)})
        d, a = {}, []
        for rowproxy in response.json():
                for column, value in rowproxy.items():
                                d = {**d, **{column: value}}
        ...
        if(len(a)>0):
                ...
                response=requests.post("http://0.0.0.0:80/api/v1/db/write",json={"table":"ride_details","column":["users","ride_Id"],"insert":[u,str(id)],"type":''})
                if(response.status_code == 200):
