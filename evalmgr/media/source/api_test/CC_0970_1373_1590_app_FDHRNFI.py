...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:''@localhost/application'
...
def add_user():
	...
	username = data['username']
	password = data['password']
	user_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"users"})
	...
	for i in range(len(user_data["username"])):
		if(username == user_data["username"][i] or not(re.match(pattern,password))):
			return jsonify({}),400
	write = requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"type":request.method,"username":username,"password":password})
	if(write):
		return jsonify({}),201
	else:
		return jsonify({}),500
...
def remove_user(username):
	...
	user_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"users"})
	user_data = user_data.json()
	ride_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"rides"})
	ride_data = ride_data.json()
	join_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"jtable"})
	join_data = join_data.json()

	for i in range(len(user_data["username"])):	
		if(username == user_data["username"][i]):
			if username in ride_data['created_by']:	
				sec_write = requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"type":"second","username":username})
			if(username in join_data['rider_list']):
				third_write = requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"type":"third","username":username})
			write = requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"type":request.method,"username":username})
			return jsonify({}),200
	else:
		return jsonify({}),400
...
def create_ride():
	...
	source=data['source']
	destination=data['destination']
	read_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"users"})
	...
	for i in range(len(read_data["username"])):
		if(created_by == read_data["username"][i] and source in enum_list and destination in enum_list):
			response = requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"type":request.method,"created_by":created_by,"timestamp":time,"source":source,"destination":destination})
			if(response):
				return jsonify({}),201
	else:
		return jsonify({}),400
...
def list_rides():
	...
	read_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"rides"})
	read_data = read_data.json()
	upcoming = []
...
def join_ride(rideId):
	...
	user_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"users"})
	user_data = user_data.json()
	jtable_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"jtable"})
	...
	if(username in user_data['username']):
		for i in range(len(ride_data['rideId'])):
			...
			if(rideId == ride_data['rideId'][i] and time1>current_time and username != ride_data['created_by'][i]):
				write = requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"type":"jtable","rideId":rideId,"username":username})
				if(write):
					return jsonify({}),200
		else:
			return jsonify({}),204
...
def delete_ride(rideId):
	if(request.method!='DELETE'):
		return jsonify({}),405
	user_data = requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"rides"})
	user_data = user_data.json()
	rideId =int(rideId)
	for i in range(len(user_data['rideId'])):	
		if rideId == user_data['rideId'][i]:
			write = requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"type":"ride","rideId":rideId})
			if(write):
				return jsonify({}),200
			return jsonify({}),400
	else:
