...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///abc1.db'
db = SQLAlchemy(app)
            ...
            if(response_check.status_code==200):
                    ...
                    if(int(source)>=1 and int(source)<=len(leng) and int(destination)>=1 and int(destination)<=len(leng) and int(source)!=int(destination)):
                        ...
                        if(len(db.session.query(ride_details).all())==0):
                                response=requests.post("http://127.0.0.1:5000/api/v1/db/write",json={"table":"ride_details","column":["created_by","timestamp","source","destination","ride_Id","users"],"insert":[created_by,timestamp,source,destination,len(db.session.query(ride_details).all())+1,created_by],"type":''})
                                return {},response.status_code
                        else:
                                response=requests.post("http://127.0.0.1:5000/api/v1/db/write",json={"table":"ride_details","column":["created_by","timestamp","source","destination","ride_Id","users"],"insert":[created_by,timestamp,source,destination,(db.session.query(db.func.max(ride_details.ride_Id)).scalar())+1,created_by],"type":''})
                                return {},response.status_code 
                    else:
                        return {},400
            else:
...
def deleteride(name):
                id=int(name)
                type="del_ride"
                response=requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"ride_details","columns":["created_by","timestamp","source","destination","ride_Id"],"where":'ride_Id='+str(id)})
                ...
                for rowproxy in response.json():
                        ...
                        a.append(d)
                if(len(a)>0):
                        res=requests.post('http://127.0.0.1:5000/api/v1/db/write',json={"table":"ride_details","column":["ride_Id"],"insert":[str(id)],"type":type})
...
def list_upcoming():
                ...
                response_check=requests.post("http://127.0.0.1:5000/api/v1/db/read",json={"table":"ride_details","columns":["created_by","timestamp","source","destination","ride_Id"],"where":'source='+source+' and destination='+destination})
...
def join_ride(id):
        ...
        response=requests.post('http://127.0.0.1:5000/api/v1/db/read',json={"table":"ride_details","columns":["created_by","timestamp","source","destination","ride_Id","users"],"where":'ride_Id='+str(id)})
        d, a = {}, []
        for rowproxy in response.json():
                for column, value in rowproxy.items():
                                d = {**d, **{column: value}}
        ...
        if(len(a)>0):
                ...
                response=requests.post("http://127.0.0.1:5000/api/v1/db/write",json={"table":"ride_details","column":["users","ride_Id"],"insert":[u,str(id)],"type":''})
                if(response.status_code == 200):
