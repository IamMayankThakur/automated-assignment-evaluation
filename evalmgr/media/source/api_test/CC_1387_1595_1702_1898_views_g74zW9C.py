...
def add_user():
	if request.method == "PUT":
		...
		if re.match('^[a-fA-F0-9]{40}$',data["password"]):
			r = post('http://127.0.0.1:80/api/v1/db/read',json={"api":"add","usn":data["username"]})
			if r.status_code == 200:
				data["api"] = "add"
				post('http://127.0.0.1:80/api/v1/db/write',json = data)
...
def list_all_upcoming_rides():
	if request.method == "GET":
		...
		if not(s and d):
			return Response(status=400)
		r = post('http://127.0.0.1:80/api/v1/db/read',json = {"api":"upcoming","src":src,"dest":dest})
...
def delete_ride(rideId):
	if request.method == "DELETE":
		r = post('http://127.0.0.1:80/api/v1/db/read',json = {"api":"delete","column":rideId})
		if r.status_code == 200:
			post('http://127.0.0.1:80/api/v1/db/write',json = {"api":"delete","column":rideId})
...
def remove_user(username):
	if request.method == "DELETE":
		r = post('http://127.0.0.1:80/api/v1/db/read',json = {"api":"remove","column":username})
		if r.status_code == 400:
			return Response(status=400)
		elif r.status_code == 200:
			post('http://127.0.0.1:80/api/v1/db/write',json = {"api":"remove","column":username})
			return Response(status=200)
	else:
		return Response(status=405)
...
def add_ride():
	if request.method == "POST":
		...
		d = False
		for a in Areas:
			if a.value == data["source"]:
				s = True
			if a.value == data["destination"]:
				d = True
		if data["source"] == data["destination"]:
			return Response(status=400)
		if not(s and d) :
			return Response(status=400)
		r = post('http://127.0.0.1:80/api/v1/db/read',json = data)
		if r.status_code == 200:
			s = post('http://127.0.0.1:80/api/v1/db/write',json = data)
...
def list_all_the_details(rideId):
	if request.method == "GET":
		r = post('http://127.0.0.1:80/api/v1/db/read',json ={"api":"list","rideid":rideId})
		if r.status_code == 200:
			return jsonify(r.json())
		else:
			return Response(status=204)
...
def joinride(rideId):
	if request.method == "POST":
		...
		r=post('http://127.0.0.1:80/api/v1/db/read',json = {"api":"join","column1":rideId,"column2":username})
		if r.status_code == 200:
			r1 = post('http://127.0.0.1:80/api/v1/db/write',json = {"api":"join","column1":rideId,"column2":username})
