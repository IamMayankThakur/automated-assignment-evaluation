...
def adduser():
	if(request.method=="PUT"):
		...
		if (Users.query.filter_by(uname=a).count() < 1):
			...
			requests.post('http://35.170.128.129/api/v1/db/write',json=dr)
...
def addride():
	if(request.method=="POST"):
		...
		if (search(source,destination)==1): #call a function to search for valid src and dst 
			...
			res=requests.post('http://35.170.128.129/api/v1/db/read',json=dr)
...
def removeRides(rideId):
	if(request.method=="DELETE"):
		...
		if Ride.query.filter_by(RideID=rideId).count() >= 1:
			...
			res=requests.post('http://35.170.128.129/api/v1/db/write',json=dr)
			return Response(json.dumps(dict()),status=200)
		else:
			return abort(400 ,description="Ride doesnot exist!")
	else:
