...
def add_user():
        if request.method == 'PUT':
                ...
                usr = {key:data[key]}
                resp_send = requests.post("http://127.0.0.1:5000/api/v1/db/read",json=usr)
...
#     data = json(d)
                print("hey")
                resp_send = requests.post("http://127.0.0.1:5000/api/v1/db/read",json=data)
                print("hi")
...
def remove_user(username):
        if request.method == 'DELETE':
                ...
                if(resp_send.status_code == 400):
                        ...
                        data["dtype"]="del_two"
                        resp_del = requests.delete("http://127.0.0.1:5000/api/v1/db/write",json=data)
                return jsonify({}),200
        else:
                return jsonify({}),405
...
def create_new_ride():
        if request.method == 'POST':
                ...
                usrd = {"username":usr}
                print(usrd)
                resp_send = requests.post("http://127.0.0.1:5000/api/v1/db/read",json=usrd)
                ...
                if(resp_send.status_code==200):
                        ...
                        if(source >= 1 and source <= n_places and destination >= 1 and destination <= n_places):
                        ...
                        fata["query"]="insert"
                        resp_send = requests.post("http://localhost:5000/api/v1/db/write",json=fata)
                        return jsonify({}),201
                else:
                        return jsonify({}),400
                return jsonify({}),500
...
def display_up_rides():
        ...
        if request.method == 'GET':
                ...
                def datet(s):
                    dat=datetime.datetime.strptime(s, '%d-%m-%Y:%S-%M-%H')
                    return dat
                qres=[]
                for i in res:
...
def details_of_rides(rideId):
        if request.method == 'GET':
                ...
                resp_send = requests.post("http://127.0.0.1:5000/api/v1/db/read",json=usr)
                resp_send1 = requests.post("http://127.0.0.1:5000/api/v1/db/read",json=data1)
                if(resp_send.status_code==200 and resp_send1.status_code==200):
                        ...
                        if usr["username"] not in d["users"]:
                                resp_send = requests.post("http://localhost:5000/api/v1/db/write",json=od)
                ...
                data = {"rideId":int(rideId)}
                resp_send1 = requests.post("http://127.0.0.1:5000/api/v1/db/read",json=data)
                if resp_send1.status_code==400:
                        return jsonify({}),400
                elif (resp_send1.status_code==200):
                        data["dtype"]="del_one"
                        resp_send = requests.delete("http://127.0.0.1:5000/api/v1/db/write",json=data)
...
def write_data():
        if request.method == 'POST':
                ...
                if data["dtype"]=='del_one':
                        ...
                        key="created_by"
                        d={key:usr}
                        resp_send = requests.post("http://127.0.0.1:5000/api/v1/db/read",json=d)
                        if (resp_send.status_code==400):
                                print("chaarsoo")
                                return jsonify({}),400
                        res = json.loads(resp_send.content)
                        ...
                        resp_send = requests.post("http://127.0.0.1:5000/api/v1/db/read",json={})
                        res = json.loads(resp_send.content)
