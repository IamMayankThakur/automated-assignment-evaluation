...
def add_user():
    if (request.method == 'PUT'):
        ...
        if ( match == None):
            ...
            d["data"] = dataDict
            d["check"] = "user"
            found = requests.post("http://127.0.0.1:8000/api/v1/db/read", data = json.dumps(d))
            found = found.text
            found = int(found)
            if (found == 1):
                return "", 400  #username already exists
            else:
                retjson = requests.post("http://127.0.0.1:8000/api/v1/db/write", data = json.dumps(d))
...
def new_ride():
    if (request.method == 'POST'):
        ...
        if found == 0:
            ...
            # d["rideid"] = 1
            d["check"] = None
            rideid = requests.post("http://127.0.0.1:8000/api/v1/db/read", data = json.dumps(d))
            rideid = rideid.text
            rideid = int(rideid)
            d["data"]["rideId"] = rideid
            retjson = requests.post("http://127.0.0.1:8000/api/v1/db/write", data = json.dumps(d))
            return "", 201
    else:
...
def upcoming_rides():
    if (request.method == 'GET'):
        ...
        if (source >= 1 and source <= 198) and (destination >= 1 and destination <= 198) and (source != destination):
            ...
            d["data"] = dataDict
            d["check"] = None
            dbquery = requests.post("http://127.0.0.1:8000/api/v1/db/read", data = json.dumps(d))
    ...
    if table != '' and table != None and work != None and work != '':
        if check == "user":
            ...
            if count == 0:
                ...
                rideid = rides.find_one(sort = [("rideId", -1)])["rideId"]
                rideid += 10
            rideid = str(rideid)
            return rideid
            ...
            for i in dbquery:
                d = dict()
                d["rideId"] = i["rideId"]   ##GLOBALLY UNIQUE CHECK
...
def db_write():
    ...
    if table != '' and table != None and work != None and work != '':
        if table == "users" and work == "INSERT":
            ...
            rides.insert({"rideId" : rideid, "created_by" : username, "timestamp" : timestamp, "source" : source, "destination" : destination, "users_rides" : [username]})
            return "Successfully Inserted"

        elif table == "rides" and work == "JOIN_EXISTING":
            rideid = data["rideId"]
            select = {"rideId" : rideid}
