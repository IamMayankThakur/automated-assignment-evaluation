...
def addUser():
    ...
    if len(ast.literal_eval(requests.post(url=api_9_read_url, json=d).text)) == 0:
        if re.search('^[a-fA-F0-9]{40}$', password):
            ...
            print('User created')
            return flask.Response(flask.jsonify({}), status=201)
        else:
...
def removeUser(name):
    ...
    d['flag'] = 'd'
    requests.post(url=api_8_write_url, json=d)
    return flask.Response(flask.jsonify({}), status=200)
...
def upcomingRides():
    ...
    if len(filtered_list) == 0:
        return flask.Response(status=204)
    return flask.Response(flask.jsonify(filtered_list), status=200)
...
def getRideDetails(rideId):
    ...
    return flask.Response(flask.jsonify(response), status=200)
