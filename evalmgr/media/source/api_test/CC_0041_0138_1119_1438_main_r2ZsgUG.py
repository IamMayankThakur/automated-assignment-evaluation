...
def create_ride():
	...
	try:
		...
		return Response(status=400)
	if all(keys in req_data for keys in app.config['ATTRIBUTES']['ride']) and validate_date(req_data['timestamp']):
...
def list_rides():
	args = request.args
	try:
		resolve_area(args['source'])
		resolve_area(args['destination'])
	except:
		return Response(status=405)
