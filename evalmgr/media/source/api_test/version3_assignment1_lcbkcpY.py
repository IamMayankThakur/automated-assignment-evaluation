...
def add_user():
    ...
    if len(ps)!=40:
        print make_response("Password is not of type SHA1 hash hex",400)
        return make_response("Password is not of type SHA1 hash hex",400)
    else:
        for i in ps:
            if(i not in cps):
                return make_response("Password is not of type SHA1 hash hex",400)
...
def get_task(task_id):             
    ...
    if(not ride_obj):
        ...
        return jsonify({"rideid":ride_dict["rideid"],"created By":ride_dict["username"],\
...
def join_ride(task_id):
    ...
    if(user1_obj): 
        if(rideid1_obj):
            ...
            return jsonify("rideid does not exist"),405 
    else:
        return jsonify("Username does not exist"), 405   
###############################################TASK 7################################################
